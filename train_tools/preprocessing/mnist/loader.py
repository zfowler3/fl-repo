...
def get_all_targets_mnist(root, dataset_label=None, mode='tr'):
    if mode == 'tr':
        train = True
    else:
        train = False
...
def get_dataloader_mnist(root, batch_size=50, dataidxs=None, dataset_label=None, mode='tr'):
    ...
    if train:
        dataset = MNIST_truncated(
            root, dataidxs, train=True, transform=train_transform, download=True
        )
        ...
        dataset = MNIST_truncated(
            root, dataidxs, train=False, transform=valid_transform, download=True
        )
        dataloader = data.DataLoader(
