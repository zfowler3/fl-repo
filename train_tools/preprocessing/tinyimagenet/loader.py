...
def get_all_targets_tinyimagenet(root, mode='tr', dataset_label=None):
    dataset = TinyImageNet_Truncated(root, mode=mode)
    all_targets = dataset.get_train_labels

    return all_targets


def get_dataloader_tinyimagenet(root, mode='tr', batch_size=50, dataset_label=None, dataidxs=None):
    train_transform, valid_transform = _data_transforms_tinyimagenet()
    if mode == 'tr':
        dataset = TinyImageNet_Truncated(
            root, mode='tr', dataidxs=dataidxs, transform=train_transform
        )
        ...
        dataset = TinyImageNet_Truncated(
            root, mode='te', dataidxs=dataidxs, transform=valid_transform
        )
        dataloader = data.DataLoader(
