...
@torch.no_grad()
def evaluate_model(model, dataloader, multilabel, device="cuda:0"):
...
@torch.no_grad()
def model_metrics(model, dataloader, previous_acc, multilabel, device="cuda:0"):
...
@torch.no_grad()
def model_metrics_glob(model, dataloader, previous_acc, multilabel, device="cuda:0"):
    """Evaluate NFR for the given dataloader"""
    model.eval()
    model.to(device)
